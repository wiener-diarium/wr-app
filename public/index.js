			...
			if (all === true) {
				if (this.classList.contains(active)) {
					...
					[...variants].forEach((el) => {
						if (
							...
							try {
								...
								console.log(
									`Error 008 in component annotation slider:                 slider class ${slider_str2} not found!`,
								);
							}
					...
					[...variants].forEach((el) => {
						if (
							...
							var selected = addMarkup(html_class2, css_class2, color2, hide2, style);
							var slider_str2 = el.opt_slider ? el.opt_slider : `${el.opt}-slider`;
							try {
								let slider = document.getElementById(slider_str2);
				...
				if (this.classList.contains(active)) {
					...
					try {
						...
						console.log(
							`Error 011 in component annotation slider:             slider class ${slider_str} not found!`,
						);
					}
					...
					try {
						...
						console.log(
							`Error 012 in component annotation slider:             slider class ${slider_str} not found!`,
						);
					}
			...
			if (urlParam.get(urlparam) == "on") {
				...
				if (!osd_test) {
					...
					var text_container_height = text_container_height_get
						...
						: 0;
					var image_container_width_get = document.getElementById(`img-resize-${image_loader_pos}`);
			...
			if (target.childNodes.length === 0) {
				...
				viewer.addHandler("open", function () {
					...
					tiledImage.getFullyLoaded()
						? hideLoading3()
						: tiledImage.addOnceHandler("fully-loaded-change", hideLoading3);
				});
			}
		}
	}
...
var UrlSearchParamUpdate = class {
	fullScreen() {
		...
		if (urlParam.get(urlparam) == "off") {
			let hide_class = document.querySelectorAll(`.${hide}`);
	...
	viewerSwitch() {
		...
		if (urlParam.get(urlparam) == "on") {
			let hide_elements = document.querySelectorAll(`.${hide}`);
			...
			let show_elements = document.querySelectorAll(`.${show}`);
			...
			let resize_elements = document.querySelectorAll(`.${resize}`);
	...
	textFeatures() {
			...
			if (url_value === null) {
				if (v.default === true) {
					...
					try {
						...
						console.log(`WARNING 7 - search_params/main: slider class ${opt_slider} not found!`);
					}
					let checkbox = document.getElementById(opt);
				...
				try {
					...
					console.log(`WARNING 8 - search_params/main: slider class ${opt_slider} not found!`);
				}
				let checkbox = document.getElementById(opt);
				if (checkbox.checked === true) {
					checkbox.checked = false;
				...
				try {
					...
					console.log(`WARNING 9 - search_params/main: slider class ${opt_slider} not found!`);
				}
				let checkbox = document.getElementById(opt);
				if (checkbox.checked === false) (checkbox.checked = true), checkbox.classList.add(active);
				...
				try {
					...
					console.log(`WARNING 10 - search_params/main: slider class ${opt_slider} not found!`);
				}
				let checkbox = document.getElementById(opt);
				if (checkbox.checked === true) {
					checkbox.checked = false;
		...
		[...variantAll].forEach((el) => {
			...
			var variants_features_class = variants.filter((v) => v.features.class === feat_leader_class);
	...
	pageUrl() {
		...
		let test_container_height_check = document.getElementById(`text-resize-${_current}`);
